...
let scenarios = [
   ...
   },  
   {
      ...
      opts:  {
              "initial":200,
              "becomeSpreader":3,"daysAsSpreader":3,
              "symptomsAppear":6,
              "infectPerDay":1,
              "percentRecorded":0.16,
              "lockdown":30,
              "spreadReduction":0.86,
              "administrativeDelay":4,
              "dateAdjust":30,
              "susceptible":2800000,
              "maxNewInfection": 330000,
              "daysOfSimulation":90,
              "showReal":1,"official":1,"simulated":1,"isLog":1
              }
    }
  ];
...
let officialData = [
	...
	{date: new Date("2020-02-09"), official: 40653},
  ...
  function round(val, scale=0.001) {
	...
	function simulate() { 
	SIMULATION_DAYS = baseOptions.daysOfSimulation; // run simulation for this number of days
	POP = baseOptions.susceptible; // area population suseptable
	MAX_NEW_INFECTION = baseOptions.maxNewInfection; 
  ...
  return copy
	...
	let accInfected = 0 ;
	  ...
	  accInfected += model[0];
		...
		let {newModel, actualPerDay} = modelForNextDay(model, day, infectPerDay*rateLimit, becomeSpreader, daysAsSpreader, dateAdjust+1, spreadReduction);
	  model = newModel;
	  if(!model) throw Error("no model");
		return {
		  day,	
...
/*           svg.selectAll("dot")
  ...
  function setOptionValues() {
	...
	return [
	  ...
	  {name: "becomeSpreader", val: 4, min: 0, max:15, title: "Spreader Day ", 
		description: "Days after infection the person become a spreader"},
	  {name: "daysAsSpreader", val: 6,min: 2, max:15, title: "Days as Spreader ", 
		description: "How many days you a spreader can infect new people before he/she gets too sick to spread"},
	  {name: "symptomsAppear", val: 9,min: 5, max:15, title: "Day Symptoms Appear ", 
	  ...
	  {name: "percentRecorded", val: 0.02, percent: true, min: 0, max: 1.0, step: 0.01, title: "Percent Recorded ", 
		description: " Percent of people who are sick that are recorded as infected"},
	  {name: "lockdown", val: 28,min: 0, max:100, title: "Lockdown day ", 
		description: "Day of lockdown from original start date for simulation"},
	  {name: "administrativeDelay", val: 2,min: 0, max:10, title: "Delay recording infection", 
		description: "Days after symptom appears and person at hospital authorities record the infection"},
	  ...
	  {name: "initial",        val: 60, min: 1, max:600, title: "Initially infected", 
	  ...
	  {name: "daysOfSimulation", val: 190,    min: 10,     max: 300,  step: 10, title: "Simul. Len", 
